...
static addrinfo	*resolve_listener_address(const ListenerConfig &config)
{
	addrinfo	hints;
	addrinfo	*result;
	int			status;
	...
	result = NULL;
	status = getaddrinfo(config.host.empty() ? NULL : config.host.c_str(),
			port_to_string(config.port).c_str(), &hints, &result);
	if (status != 0)
...
static int	try_open_socket(const ListenerConfig &config, const addrinfo *address)
{
	int fd;
	...
	try
	{
		...
		if (bind(fd, address->ai_addr, address->ai_addrlen) < 0)
			throw std::runtime_error(socket_error("bind failed for "
					+ config.host + ":" + port_to_string(config.port)));
...
static ListenerSocket	create_listener_socket(const ListenerConfig &config)
{
	addrinfo	*addresses;
	addrinfo	*current;
	int			fd;
...
ListenerSocket::ListenerSocket(int fd, const ListenerConfig &config,
...
void	ListenerManager::openAll(const std::vector<ListenerConfig> &configs)
{
	closeAll();
	if (configs.empty())
