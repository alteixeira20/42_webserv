...
{
	std::vector<ListenerConfig> endpoints;
	endpoints.push_back(ListenerConfig("127.0.0.1", 0));
	endpoints.push_back(ListenerConfig("127.0.0.1", 0));
	...
	for (std::size_t i = 0; i < manager.listeners().size(); ++i)
	{
		...
		assert_true(manager.listeners()[i].config.host == endpoints[i].host,
			"expected listener to keep its host config");
		assert_true(manager.listeners()[i].config.port == endpoints[i].port,
